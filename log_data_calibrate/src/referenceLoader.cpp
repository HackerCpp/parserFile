...
ReferenceLoader::ReferenceLoader(int lines,QString data_step)
    ...
    m_model->setQuery(QString("SELECT SpectrumCalibReference.SCReferenceID,CalibDevice.CDName,CalibDevice.CDNumber,\
                      SpectrumCalibReference.SCRLastChanges,SpectrumCalibReference.SCRCurveMaxID,\
                      SpectrumCalibReference.SCRCurveMinID,SpectrumCalibReference.SCRCurveNoiseID,SpectrumCalibReference.SCRLines,SpectrumCalibReference.SCRDataStep,SpectrumCalibReference.SCRComment\
                 ...
                 SpectrumCalibReference.SCRDataStep = '%2' AND SpectrumCalibReference.SCRStatus='%3';").arg(lines).arg(data_step).arg(ReferenceStatus::STATUS_REFERENCE)
                );
    /*(m_model->setQuery("\
...
void ReferenceLoader::loadRefCurves(QVector<ICurve*> &referenceSpectrums,int &idReference){
    while(m_currentCurveIndex == -1){
        qApp->processEvents();
    }

    if(m_currentCurveIndex >= 0){
        idReference = m_model->data(m_model->index(m_currentCurveIndex,0)).toInt();
        int indexCurveMaximum = m_model->data(m_model->index(m_currentCurveIndex,4)).toInt();
        int indexCurveMinimum = m_model->data(m_model->index(m_currentCurveIndex,5)).toInt();
        int indexCurveNoise = m_model->data(m_model->index(m_currentCurveIndex,6)).toInt();

        auto f_loaderDB = std::make_unique<SQLite3Loader>(m_db);
        referenceSpectrums[MAXIMUM_SPECTRUM] = f_loaderDB->loadCurve(indexCurveMaximum);
        referenceSpectrums[MINIMUM_SPECTRUM] = f_loaderDB->loadCurve(indexCurveMinimum);
        referenceSpectrums[NOISE_SPECTRUM] = f_loaderDB->loadCurve(indexCurveNoise);
    }
}
